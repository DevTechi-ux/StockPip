...
export async function POST(
  ...
  try {
    ...
    const transactionResult = await executeQuery(
      ...
      [transactionId, userId, account.id, 'ADMIN_ADJUSTMENT', addAmount, currentBalance, newBalance, 'USD', description || 'Admin fund addition']
    );
    ...
    if (transactionResult.success) {
      await executeQuery(
        ...
        [
          ...
          `$${addAmount.toFixed(2)} has been added to your trading account. Your new balance is $${newBalance.toFixed(2)}.`,
          JSON.stringify({
    ...
    return NextResponse.json({
      ...
      message: transactionResult.success ? 'Funds added successfully' : 'Failed to record transaction'
    });
  } catch (error) {
